    ...
    <div class="card-body">
      <div class="row">
        ...
        <div class="col-lg-12" id="pageForm" style="display: none;">
          <div class="card">
            <div class="card-body">
              <h2 class="card-title"><span id="Ingiza" style="display: none;">Ingiza bodi Mpya</span><span
                  id="Rekebisha" style="display: none;">Rekebisha bodi</span> <button
                  class="btn btn-primary float-right" onclick="showForm('Ingiza')">Ona bodi</button></h2>
              <form action="{{route('admin-process-bodi-za-usimamizi')}}" method="post" id="fullForm">
                @csrf
                <input type="hidden" name="action" value="add" required id="formAction">
                <input type="hidden" name="id" value="" id="formId">
                <div class="form-group">
                  <label for="title">Jina la bodi</label>
                  <input type="text" class="form-control" name="title" id="title" placeholder="" required>
                </div>
                <div class="form-group">
                  <label for="details">Maelezo ya bodi</label>
                  <textarea class="form-control" name="details" id="details" rows="4" required></textarea>
                </div>
                <input type="submit" class="btn btn-success btn-fw mr-2" value="Ingiza bodi">
        ...
        <div class="col-lg-12" id="pageList">
          <div class="card">
            <div class="card-body">
              <h2 class="card-title">Orodha ya bodi <button class="btn btn-primary float-right"
                  onclick="showForm('Ingiza')">Ingiza bodi</button></h2>
              <div class="accordion accordion-bordered" id="bodi_parent">
                @foreach ($data as $i => $h)
          <div class="card p-0 m-0">
            ...
            <div id="collapse-{{$i}}" class="collapse show" data-parent="#bodi_parent">
            <div class="card-body px-4">
              {{$h->details}}
              <div class="mt-2">
              <button class="btn btn-outline-primary btn-fw mr-2"
                onclick="showForm('Rekebisha',{{json_encode($h)}})">Edit</button>
              <form action="{{route('admin-process-bodi-za-usimamizi')}}" method="POST" style="display:inline;"
